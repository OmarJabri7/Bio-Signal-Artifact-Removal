...
for sweeper in range(sweepFrom, sweepTo):
    ...
    print("moving average: alpha: ",
          moving_average_alpha[1], " delta: ", moving_average_delta[1])
    ...
    if doParamSweep == 1:
        ...
        print("saving SNRs")
        iteration = 1
        correction_a = np.loadtxt(
            '../cppData{}/py_alpha_coefficient.txt'.format(fileName[myData]), int)
        correction_d = np.loadtxt(
            '../cppData{}/py_delta_coefficient.txt'.format(fileName[myData]), int)
        goodOnes = np.loadtxt(
            '../cppData{}/py_goodOnes.txt'.format(fileName[myData]), int)
    ...
    alpha_all_included = alpha_all_included_orig.reshape(
        (num_subject + 1), iteration)
    delta_all_included = delta_all_included_orig.reshape(
        (num_subject + 1), iteration)
    ...
    for j in range(iteration):
        ...
        for k in subject_array:
            ...
            fft_outer_closed = np.abs(np.fft.fft(outer_closed)[
                                      0:np.int(dataLength / 2)])
            fft_inner_closed = np.abs(np.fft.fft(inner_closed)[
                                      0:np.int(dataLength / 2)])
            fft_closed_nn = np.abs(np.fft.fft(closed_nn)[
                                   0:np.int(dataLength / 2)])
            fft_outer_opened = np.abs(np.fft.fft(outer_opened)[
                                      0:np.int(dataLength / 2)])
            fft_inner_opened = np.abs(np.fft.fft(inner_opened)[
                                      0:np.int(dataLength / 2)])
            fft_opened_nn = np.abs(np.fft.fft(opened_nn)[
                                   0:np.int(dataLength / 2)])
            ...
            fft_inner_opened_smooth_alpha = df_inner_opened.y.rolling(
                window=int(moving_average_alpha[subject])).mean()
            fft_inner_closed_smooth_alpha = df_inner_closed.y.rolling(
                window=int(moving_average_alpha[subject])).mean()
            fft_opened_nn_smooth_alpha = df_opened_nn.y.rolling(
                window=int(moving_average_alpha[subject])).mean()
            fft_closed_nn_smooth_alpha = df_closed_nn.y.rolling(
                window=int(moving_average_alpha[subject])).mean()
            fft_inner_opened_smooth_delta = df_inner_opened.y.rolling(
                window=int(moving_average_delta[subject])).mean()
            fft_inner_closed_smooth_delta = df_inner_closed.y.rolling(
                window=int(moving_average_delta[subject])).mean()
            fft_opened_nn_smooth_delta = df_opened_nn.y.rolling(
                window=int(moving_average_delta[subject])).mean()
            fft_closed_nn_smooth_delta = df_closed_nn.y.rolling(
                window=int(moving_average_delta[subject])).mean()
            ...
            noise_alpha = np.sum(
                fft_inner_opened[alphaStart:alphaEnd]) / diff_noise_alpha
            integral_alpha = np.sum(
                fft_inner_closed_scaled_alpha[alphaStart:alphaEnd]) / diff_noise_alpha
            ...
            noise_alpha_nn = np.sum(
                fft_opened_nn[alphaStart:alphaEnd]) / diff_noise_alpha
            integral_alpha_nn = np.sum(
                fft_closed_nn_scaled_alpha[alphaStart: alphaEnd]) / diff_noise_alpha
            ...
            signal_delta = np.sum(
                fft_inner_closed_scaled_delta[deltaStart:deltaEnd]) / diff_noise_delta
            integral_delta = np.sum(
                fft_inner_opened[deltaStart:deltaEnd]) / diff_noise_delta
            ...
            signal_delta_nn = np.sum(
                fft_closed_nn_scaled_delta[deltaStart:deltaEnd]) / diff_noise_delta
            integral_delta_nn = np.sum(
                fft_opened_nn[deltaStart:deltaEnd]) / diff_noise_delta
            ...
            if doParamSweep == 0:
                SNRarray = np.array(
                    [snr_alpha, snr_alpha_nn, snr_alpha_ratio, snr_delta, snr_delta_nn, snr_delta_ratio])
    ...
    if doParamSweep == 1:
        ...
        goodOnes = np.where((delta_max_coefficient < 3) &
                            (alpha_max_coefficient < 3))[0]
        np.savetxt('../cppData' +
                   fileName[myData] + '/py_goodOnes.txt', goodOnes, fmt='%d')
        np.savetxt('../cppData' + fileName[myData] +
                   '/py_alpha_coefficient.txt', alpha_max_coefficient, fmt='%d')
        np.savetxt('../cppData' + fileName[myData] +
                   '/py_delta_coefficient.txt', delta_max_coefficient, fmt='%d')

    if doParamSweep == 0:
        alpha_max = alpha_matrix
        alpha_max_coefficient = correction_a[np.where(
            (correction_a < 3) & (correction_d < 3))]
        delta_max = delta_matrix
        delta_max_coefficient = correction_d[np.where(
            (correction_a < 3) & (correction_d < 3))]
    ...
    print()
    print("----> ", len(np.where(alpha_max > 0)
          [0]), " alpha with sum of: ", np.sum(alpha_max))
    print("----> ", len(np.where(delta_max > 0)
          [0]), " delta with sum of: ", np.sum(delta_max))
