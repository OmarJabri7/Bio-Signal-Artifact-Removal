...
# sin2eeg *= gain
# eeg_noise *= gain
# sig_eeg = np.column_stack((sin2eeg, eeg_noise))
...
# #                  index=True, header=False,  sep="\t")
# # Try other dataset
# dat_tsv = np.loadtxt('Data/Novel_Subject.tsv')
# inner = dat_tsv[:, 1]
# outer = dat_tsv[:, 2]
# # # Inner
# N = min(inner.shape[0], eeg_dat.shape[0])
# fs = 1000
# T = 1/fs
# t_in = np.arange(N)*T
# eyes_closed_inner = inner[:N]
# dic_fp1 = fit_sin(t_in[:N], inner[:N])
# fft_eeg = np.fft.fft(eyes_closed_inner)
# mean_eeg = np.mean(inner)
# std_eeg = np.std(inner)
# amp_eeg = np.abs(fft_eeg/N)**2
# phi_eeg = 2*np.angle(fft_eeg)/N
# eeg_freqs = np.linspace(0, fs/2, floor(N/2) + 1)
...
# print(dic_fp1["amp"], dic_fp1["omega"], dic_fp1["phase"], dic_fp1["offset"])
# yf = np.fft.fft(eyes_closed_inner)
# freq = np.fft.fftfreq(eyes_closed_inner.size, d=T)
# magnitude = np.abs(yf)
# phase = np.angle(yf)
# print(np.mean(magnitude), np.mean(freq), np.mean(phase))
# plt.plot(t_in[:N], eyes_closed_inner[:N])
# plt.plot(t_in[:N], sin2fp1[:N])
# plt.xlabel("Time (s)")
# plt.ylabel("Signal Voltage (V)")
# plt.title(f"Time Domain Optimal fit Sinewave vs Real EEG")
# noise_lvl = 0.01
